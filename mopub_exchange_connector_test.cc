/* mopub_exchange_connector_test.cc
   Murtaza Hussain, 02 July 2013
   

   Exchange connector for MoPub.
*/
...
/*****************************************************************************/
/* TEST MOPUB EXCHANGE CONNECTOR                                           */
/*****************************************************************************/

/** This class extends the standard MoPub exchange connector with
    functionality needed for MoPub's integration test page.

    It includes the following extra functionality:
    * 
*/

struct TestMoPubExchangeConnector: public MoPubExchangeConnector {

    TestMoPubExchangeConnector(const std::string & name,
                             std::shared_ptr<ServiceProxies> proxies)
        : MoPubExchangeConnector(name, proxies)
    {
    }
                             ...
                             string format) const
    {
...

        {
            ...
            static const Cairo::RefPtr<Cairo::Surface> logo
                = Cairo::ImageSurface::create_from_png("rtbkit/static/img320-50.png");
...
//      float decodedPrice = encodedPrice;
...
BOOST_AUTO_TEST_CASE( test_mopub_decode_price )
{
    float price = MoPubExchangeConnector::decodeWinPrice("hheehhee", "386C13726472656E");
...
BOOST_AUTO_TEST_CASE( test_mopub )
{
    ...
    // 10003.  Note that we need to ensure that port 10003 is open on
    // our firewall.
    std::shared_ptr<TestMoPubExchangeConnector> connector
        (new TestMoPubExchangeConnector("connector", proxies));
    ...
    //connector->configureHttp(1, port, "54.214.251.166");
    connector->start();
...
//    std::string hostName = ML::fqdn_hostname(portName) + ":" + portName;
    std::string hostName = std::string("54.214.251.166:") +  portName;

    agent.config.providerConfig["mopub"]["seat"] = "123";

    // Configure the agent for bidding
    for (auto & c: agent.config.creatives) {
        c.providerConfig["mopub"]["adomain"][0] = "54.214.251.166";
		c.providerConfig["mopub"]["adm"]
            = "<script type=\"text/javascript\"><img src=\"http://"
            + hostName
            + "/creative.png?width="
            + to_string(c.format.width)
            + "&height="
            + to_string(c.format.height)
            + "&price=${AUCTION_PRICE:BF}\"/></script>";
		
		c.providerConfig["mopub"]["crid"] = c.name;
  
//cerr <<"info" <<c.providerConfig["mopub"]["adm"]<< endl;

		c.providerConfig["mopub"]["attr"][0] = 0;
        c.providerConfig["mopub"]["adid"] =  c.id; //Ad ID
        c.providerConfig["mopub"]["iurl"] = "http://"
                                            + hostName
                                            + "/creative.png?width="
                                            + to_string(c.format.width)
                                            + "&height="
...
cerr << "Bid Available Creatives" << bid.availableCreatives[0]<< endl;
    ...
    std::string filename = "rtbkit/plugins/exchange/testing/mopub-samples.txt.gz";
